...
function getRightCaptain() {
    ...
    let result;
    for (key in crewObj) {
      if (crewObj[key]['Должность'] == 'Капитан' && Number(crewObj[key]['Стаж']) > experience) {
        experience = Number(crewObj[key]['Стаж']);
        result = crewObj[key]['Имя'] + ", " + crewObj[key]['пол (м/ж)'] + ", " + crewObj[key]['Должность'] + ", " + crewObj[key]['Стаж'];
    }
  }
    return result;
}
...
function getRightDoc() {
    ...
    let result;
    for (key in crewObj) {
      if (crewObj[key]['Должность'] == 'Врач' && crewObj[key]['пол (м/ж)'] == 'ж' && Number(crewObj[key]['Стаж']) > experience) {
        experience = Number(crewObj[key]['Стаж']);
        result = crewObj[key]['Имя'] + ", " + crewObj[key]['пол (м/ж)'] + ", " + crewObj[key]['Должность'] + ", " + crewObj[key]['Стаж']
    }
  }
...
function getAllEngineer() {
  ...
  for (key in crewObj) {
    if (crewObj[key]['Должность'] == 'Бортмеханик') {
      result.push(crewObj[key]['Имя'] + ", " + crewObj[key]['пол (м/ж)'] + ", " + crewObj[key]['Должность'] + ", " + crewObj[key]['Стаж']);
  }
}
  return result;
}
...
function getAllRover(){
  ...
  for (key in equipmentObj) {
    if (equipmentObj[key]['Тип оборудования'] == 'марсоход') {
      result.push(equipmentObj[key]['Название оборудования'] + ", " + equipmentObj[key]['Тип оборудования'] + ", " + equipmentObj[key]['Предполагаемый срок службы (в годах)']);
  }
}
  return result;
}
...
function getRightRovers(){
  let result = [];
  for (key in equipmentObj) {
    if (equipmentObj[key]['Тип оборудования'] == 'марсоход' && Number(equipmentObj[key]['Предполагаемый срок службы (в годах)']) > 3) {
      result.push(equipmentObj[key]['Название оборудования'] + ", " + equipmentObj[key]['Тип оборудования'] + ", " + equipmentObj[key]['Предполагаемый срок службы (в годах)']);
  }
}
  return result;
  ...
  let range = 0;
    ...
    for (key in rocketsObj) {
      if (Number(rocketsObj[key]['дальность полета (тыс. км.)']) > range) {
        ...
        result = rocketsObj[key]['Название ракеты'] + ", " + rocketsObj[key]['тип ракеты'] + ", " + rocketsObj[key]['дальность полета (тыс. км.)'];
    }
  }
    return result;
}
