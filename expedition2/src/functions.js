...
function getRightEngineer() {
  ...
  let result = {};
  for (key in crewObj) {
    if (crewObj[key]['Должность'] == 'Бортмеханик' && crewObj[key]['пол (м/ж)'] == 'ж' && Number(crewObj[key]['Стаж']) > experience) {
      experience = Number(crewObj[key]['Стаж']);
      result = crewObj[key];
  }
}
...
function getRightDoc() {
  let result = [];
  for (key in crewObj) {
    if (crewObj[key]['Должность'] == 'Врач' && Number(crewObj[key]['Стаж']) > 5) {
...
function getRightRovers(){
  let result = [];
  for (key in equipmentObj) {
    if (equipmentObj[key]['Тип оборудования'] == 'луноход' && Number(equipmentObj[key]['Предполагаемый срок службы (в годах)']) > 7) {
      result.push(equipmentObj[key]);
  }
}
  return result;
